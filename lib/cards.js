...
export function isHoldingItem(card, cardInfo, config, type) {
  ...
  const category = (cardInfo.rarity || cardInfo.category || '').toLowerCase();
  ...
  let multiplierMet = false;
  let effectiveMV = marketValue;
  
  if (isBid) {
    // Bids: price <= threshold and/or price <= marketValue / multiplier
    thresholdMet = price <= config.holdingItemThresholdBid;
    multiplierMet = marketValue > 0 && (price <= marketValue / multiplier);
  } else {
    // Asks: use MV floor of 30 for legendaries
    if (category === 'legendary') {
      effectiveMV = Math.max(marketValue, 30);
    }
    multiplierMet = effectiveMV > 0 && (price >= effectiveMV * multiplier);
  }
  
  // Age check (separate thresholds for bids and asks)
  const ageThreshold = isBid ? config.holdingItemAgeHoursBid : config.holdingItemAgeHoursAsk;
  const ageMet = ageHours >= ageThreshold;
  
  // Logic: for bids - threshold AND (multiplier OR age)
  //        for asks - multiplier OR age
  let isHoldingItem;
  if (isBid) {
    isHoldingItem = thresholdMet && (multiplierMet || ageMet);
  } else {
    isHoldingItem = multiplierMet || ageMet;
  }
  ...
  return {
    ...
    effectiveMV,
    category,
    threshold: isBid ? config.holdingItemThresholdBid : null,
    multiplier
  };
}
